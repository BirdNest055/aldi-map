...
export async function POST(req: NextRequest) {
  ...
  // ALDI: fetch inline, save as national (one set for all ALDI stores)
  if (store.brand === "aldi-sued") {
    const registry = getRegistry();
    const plugin = registry.getPluginForStore(store);
    if (!plugin) {
      return NextResponse.json({ error: `No fetcher for brand "${store.brand}"` }, { status: 400 });
    }
    try {
  ...
  // REWE: trigger GitHub Actions workflow (async)
  if (store.brand === "rewe") {
    ...
    if (!offersUrl) {
      return NextResponse.json({ error: "No offers URL for this REWE store" }, { status: 400 });
    }
    try {
      const triggerRes = await fetch(
        `https://api.github.com/repos/${repo}/actions/workflows/${workflowId}/dispatches`,
        {
          ...
          body: JSON.stringify({ ref: "main", inputs: { store_url: offersUrl, store_id: storeId } }),
        }
      );
      if (!triggerRes.ok) {
        const errText = await triggerRes.text();
        return NextResponse.json({ error: `Failed to trigger REWE workflow: ${errText}` }, { status: 502 });
      }
      return NextResponse.json({
        success: true,
        storeId: storeId,
      ...
      return NextResponse.json({ error: `Workflow trigger failed: ${e.message}` }, { status: 500 });
    }
  }

  return NextResponse.json({ error: `Unsupported brand: ${store.brand}` }, { status: 400 });
}
